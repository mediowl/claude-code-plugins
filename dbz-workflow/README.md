...
# またはユーザースコープ（個人環境）
claude plugin install dbz-workflow@mediowl-plugins --scope user
```
...
dbz-workflow には以下の Hooks が含まれていますが、プラグインの Hooks は Claude Code が自動的に読み込みません。有効にするには、`settings.json` に手動で登録する必要があります。
...
Claude Code 内で `/hooks` を実行し、対話式で Hooks を追加してください。

**SessionStart に追加する内容:**
...
`.claude/settings.json`（プロジェクトスコープ）または `~/.claude/settings.json`（ユーザースコープ）に以下を追加してください。既に `hooks` キーがある場合は、既存の設定にマージしてください。
...
> **注意**: Hooks の登録状況は `/dbz-plan`, `/dbz-pr`, `/dbz-team` の実行時に自動チェックされます。未登録の場合は案内メッセージが表示されます。
...
```bash
claude plugin uninstall dbz-workflow@mediowl-plugins
```
